...
- Firebase Analytics (GA4) lives in `src/lib/analytics.ts`: `getAnalyticsInstance()` (memoized, browser-only, prod-only, `isSupported()`-guarded, dynamically imported, never throws) plus a fire-and-forget `trackEvent(name, params)`. `src/components/FirebaseAnalytics.tsx` just calls it on mount, so init and event logging share one Firebase app. Track a CTA with `TrackedButton` (`src/components/ui/TrackedButton.tsx`) — `Button` and the sections are server components and cannot take an `onClick`. Don't add a second analytics integration without removing this one.
...
- CI runs exactly `npm install && npm run lint && npm run build` on Node 24.18.0
  (`.github/workflows/landing-ci.yml`). If those two pass locally, CI passes.
  It is `npm install`, NOT `npm ci`, on purpose: `npm ci` hard-fails on any
  lockfile/package.json drift, which this repo wants reconciled rather than
  fatal. Reproducing CI with `npm ci` will fail on drift CI would have fixed.
